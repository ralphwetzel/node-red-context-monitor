...
// RAW_TARGET lets us identify our own proxies; unwrap() peels them (incl. nested
// occurrences) before a value is written to a store.

const RAW_TARGET = Symbol("node-red-context-monitor.raw-target");

let unwrap = function(value) {

    let peel = function(v) {
        while (v !== null && typeof v === "object" && v[RAW_TARGET]) {
            v = v[RAW_TARGET];
        }
        return v;
    }
    ...
    // walk the object graph to peel nested proxies; guard against cycles
    const seen = new WeakSet();

    let walk = function(obj) {
