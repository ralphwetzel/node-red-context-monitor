...
function init(_RED, cache) {
    RED = _RED;
    monitors = cache;

    RED.util.contextMonitor ??= {};

    if (RED.util.contextMonitor.getRaw) {
        RED.log.warn("ContextMonitor: RED.util.contextMonitor.getRaw already defined.");
        return;
    }

    Object.defineProperty(RED.util.contextMonitor, "getRaw", {
        value: unwrapProxy,
        enumerable: false,
        configurable: true
    });
...
// *****
// Fix for proxy accumulation:
// Values obtained via the monitoring 'get' are Proxies. If a flow writes such a
// value back into context (the canonical get -> modify -> set pattern), the Proxy
// itself used to be persisted into the store - and re-wrapped on every following
// get/set cycle, growing an ever deeper proxy chain until the process dies with
// 'RangeError: Maximum call stack size exceeded'.
// rawTargetByProxy lets us identify our own proxies; unwrapProxy() peels proxy
// chains before a value is written to a store or inserted into a stored object.

const rawTargetByProxy = new WeakMap();

function unwrapProxy(value) {

    while (value !== null && typeof value === "object") {
        const target = rawTargetByProxy.get(value);
        if (!target) {
            return value;
        }
        value = target;
    ...
    Object.defineProperties(wrapper, {
        get: {
            value: function(key, storage, callback) {
                ...
                let create_object_wrapper = function(obj, getter_key) {
                    ...
                    const proxy = new Proxy(obj, {
                        get: (target, property, receiver) => {
                            ...
                            if (Array.isArray(target) && mutatingArrayMethods.has(property)) {
                                return (...args) => {
                                    ...
                                    trigger(root_id ?? node_id, context_id, key, value, previous_value, prop_chain, options);
                        ...
                        set: (target, propertyKey, value, receiver) => {
                            ...
                            // unwrap monitoring proxies: never persist them into the target
                            value = unwrapProxy(value);

                            // this is the monitoring function!
                            const previous_value = target[propertyKey];
                        ...
                        deleteProperty(target, propertyKey) {
                            ...
                            if (undefined !== previous_value) {
                                ...
                                trigger(root_id ?? node_id, context_id, key, undefined, previous_value, prop_chain, options);
                            }
                    ...
                    rawTargetByProxy.set(proxy, obj);
                ...
                if (callback) {
                    ...
                    let callback_wrapper = function(err, ...values) {
                        if (err) {
                            callback(err);
                            return;
                        }
                        ...
                        if (Array.isArray(key)) {
                            const results = values.map((value, index) => {
                                return create_object_wrapper(value);
                            });
                        ...
                        callback(undefined, create_object_wrapper(values[0]));
                    }
                ...
                // ATT: if typeof key is array, context.get returns an array of values!
                let getted_value = context.get(key, storage);

                if (Array.isArray(key) && Array.isArray(getted_value)) {
                    return getted_value.map((value, index) => create_object_wrapper(value));
                }
        ...
        set: {
            value: function(key, value, storage, callback) {
                ...
                // unwrap monitoring proxies: never persist them into the store
                value = unwrapProxy(value);

                let previous_value = context.get(key, storage);

                let trigger_set = function() {
                    if (Array.isArray(key)) {
                        const values = Array.isArray(value) ? value : [value];

                        key.forEach((single_key, index) => {
                            const new_value = index < values.length ? values[index] : null;
                            const old_value = Array.isArray(previous_value) ? previous_value[index] : undefined;

                            trigger(root_id ?? node_id, context_id, single_key, new_value, old_value, undefined, storage);
                        });
                        return;
                    }
                ...
                if (typeof callback === 'function') {
                    const callback_wrapper = function(err, res) {
                        if (!err) {
                            trigger_set();
                        }
                    ...
                    // Delegate to Node-RED's real async context API and only
                    // emit monitoring notifications after the write succeeds.
                    return context.set(key, value, storage, callback_wrapper);
                }

                const res = context.set(key, value, storage);
                trigger_set();
                return res;
            }
        },
...
// options:
// - storage:   (string)    the context store to use
// - changed:   (boolean)   pre-calculated info that new_value is different from previous_value. Only present if "true".

let trigger = function(root_id, context_id, key, new_value, previous_value, prop_chain, options) {
    ...
    function trigger_receivers(monitoring_nodes, message) {
        ...
        monitoring_nodes.forEach(node => {
            ...
            // BUT: structuredClone fails on Proxied Objects!
            // Until we implemented a workaround, we keep cloneMessage !!
            // let msg = structuredClone(message);
    ...
    for (let i=keys.length; i>0; i--) {
        ...
        if (mons.length) {
            let msg = {
                ...
                changed: changed ?? new_value != previous_value
            }
    ...
    if ("object" === typeof new_value) {
        ...
        const trigger_kids = function(parent_id, kids, new_value, previous_value, key_chain) {
            ...
            kids.forEach(kid => {
                ...
                let msg = {
                    ...
                    // As we're walking deeper into the object,
                    // we're not using options.changed here,
                    // but do the check at each step.
                    changed: nv != pv
                }
        ...
        if (search_id in monitors) {
            // If there are monitoring nodes for nested properties, the entry point is the
            // current key itself. It might yet be that it has no kids => no downstream monitors.
